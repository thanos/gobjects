...
IMAGE_NAME := "thanos/gobjects"

default: test

help:
	@echo 'Management commands for gobjects:'
	@echo
	@echo 'Usage:'
...
build-docker:
	@echo "building ${BIN_NAME} ${VERSION}"
	docker build -t gobjects:build -f Dockerfile.build .
	docker run --name=gobjects -v $(GOPATH):/gopath/  gobjects:build
	docker rm -f gobjects
